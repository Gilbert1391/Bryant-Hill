...
// Nodelist elements
var imageElems = document.querySelectorAll("." + images);
var overlayButtons = document.querySelectorAll('#overlayPrev, #overlayNext');

// Replace overlay img
var overlay = function overlay(img) {
  return document.querySelector('.overlay__img').src = img.src;
};
var currentOverlay = void 0;
...
query.addEventListener('click', function (event) {
  ...
  var index = Array.prototype.indexOf.call(query.children, target);
...
overlayButtons.forEach(function (el) {
  return el.addEventListener('click', function () {
    var end = imageElems.length - 1;
    var direction = event.target.id === 'overlayPrev' ? -1 : 1;
